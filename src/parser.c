...
static inline bool is_function_call(Token* current_token) {
	/* Usage of NEXT_TOKEN here is not safe here, should probably make use of IS_END_TOKEN and IS_START_TOKEN */
	return strcmp(NEXT_TOKEN(current_token).value, FUNC_OPEN) == 0;
}
...
void parse_function_call(State* state, Token* token) {
	...
	while (strcmp(token->value, FUNC_CLOSE) != 0) {
		if (strcmp(token->value, ARG_SEPARATOR) != 0) {
			arguments[length++] = parse_value(token);
		}
		++token;
	}
